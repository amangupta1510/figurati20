				...
				<div class="pmd-card-body">
					...
					<div class="group-fields clearfix row">
						<div class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
							<div class="lead"><a>PRODUCT DETAILS<a>&nbsp;<a class="export_excel btn btn-primary" style="padding: 6px 6px;font-size: 13px;">Download Excel <i class="fas fa-download"></i></a>
...
<script type="text/javascript">
  function date_clear() {
	...
	var end = $('#end').val();
	if(start!=''&&end!=''){
   	var img ="{{ route('admin-product_details_load',['id'=>':id','type'=>':page','start'=>':start','end'=>':end']) }}";
...
$('input[type="radio"]').on('change', function(e) {
   if($(this).val()=='all'){
   	$('#date_div').hide();
   	var img ="{{ route('admin-product_details_load',['id'=>':id','type'=>':page']) }}";
...
if(val=='all'){
	var img ="{{ route('admin-export_product_details',['id'=>':id','type'=>':page']) }}";
    var img = img.replace('%3Apage','all');
    var img = img.replace('%3Aid',"{{app('request')->input('id')}}")
    var img = img.replace('&amp;','&');
  ...
  var end = $('#end').val();
  if(start!=''&&end!=''){
   	var img ="{{ route('admin-export_product_details',['id'=>':id','type'=>':page','start'=>':start','end'=>':end']) }}";
    var img = img.replace('%3Apage','date');
    var img = img.replace('%3Aid',"{{app('request')->input('id')}}")
    var img = img.replace('&amp;','&');
